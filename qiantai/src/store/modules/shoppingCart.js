...
export default {
  ...
  getters: {
    ...
    getTotalPrice (state) {
      ...
      for (let i = 0; i < state.shoppingCart.length; i++) {
        const temp = state.shoppingCart[i];
        if (temp.check) {
          totalPrice += temp.shangpin.jiage * temp.num;
        }
      }
      return totalPrice;
    }
  ...
  mutations: {
    ...
    updateShoppingCart (state, payload) {
      ...
      if (payload.prop == "num") {
        ...
        if (state.shoppingCart[payload.key].shangpin.kucun < payload.val) {
          return;
        }
        if (payload.val < 1) {
          return;
    ...
    addShoppingCartNum (state, shangpinID) {
      // 增加购物车商品数量
      // 用于在商品详情页点击添加购物车,后台返回002，“该商品已在购物车，数量 +1”，更新vuex的商品数量
      for (let i = 0; i < state.shoppingCart.length; i++) {
        const temp = state.shoppingCart[i];
        if (temp.shangpinID == shangpinID) {
  ...
  actions: {
    ...
    addShoppingCartNum ({ commit }, shangpinID) {
      commit('addShoppingCartNum', shangpinID);
    },
